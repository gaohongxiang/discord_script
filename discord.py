...
from config import discord_file, discord_fix_messages_file, discord_channel_messages_file
...
class DiscordUtil():
    ...
    @try_except_code
    def send_message(self, token, proxy, message, to_channel, guild_id='', reply_message_id='', reply_account_id='', reply_rate=0.4):
        """发送或者回复聊天
        ...
        """
        account_id,_ = self.get_account_id(token,proxy)
        header = {
            "Authorization":str(token),
            "Content-Type":"application/json",
        }
        ...
        if 'code' in response:
            ...
            if response['message'] == 'This action cannot be performed due to slowmode rate limit':
                print('慢速模式速率限制,稍后重试')
                return
            # 有时候别人在频道发送消息太多，会限制。不是自己发送的消息多，这种情况就重试
            self.send_message(token, proxy, message, to_channel, guild_id, reply_message_id, reply_account_id, reply_rate)
        else:
    ...
    def listening(self, token, account_id, account_username, channel_id, proxy, mods, keywords, from_email, to_email):
        ...
        for data in response:
            ...
            if (data['mention_everyone'] == True) or (account_id in data['content']) or ((data['author']['id'] in mods) and (keyword for keyword in keywords if keyword in data['content'])):
                # 信息发送到邮箱
    ...
    def chat(self, account_info, from_channel_list, to_channel, from_email, to_email, guild_id='', reply_rate=0.4, interval_time=60, is_channel_message=True, mods=[], keywords=[]):
        ...
        while True:
            ...
            mentioned = self.listening(token, account_id, account_username, to_channel, proxy, mods, keywords, from_email, to_email)
            if mentioned == True:
                break
            # 发送消息或者回复消息。根据回复概率决定要不要回复消息
            reply_message_id, reply_account_id = self.send_message(token, account_id, proxy, message, to_channel, guild_id, reply_message_id, reply_account_id, reply_rate)
            # 休眠
